...
const TutorCard = ({ tutor }) => {
  ...
  const averageRating = tutor.numRatings > 0 ? (tutor.rating / tutor.numRatings).toFixed(1) : "No ratings yet";
  ...
  return (
    <div className="card bg-base-100 shadow-xl transition-transform hover:scale-105">
      <div className="card-body">
        ...
        {/* Use the 'bio' field */}
        <p className="text-gray-600 h-20 overflow-hidden text-ellipsis">{tutor.bio}</p>
        ...
        <div className="my-2">
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            ...
            {tutor.classes?.length > 3 && <div className="badge badge-outline">...</div>}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
            <div className="rating rating-sm">
                ...
                <input type="radio" name="rating-1" className="mask mask-star" disabled checked />
            </div>
        ...
        <div className="card-actions justify-end">
          ...
          <Link href={`/tutors/${tutor.id}`} className="btn btn-primary">View Profile</Link>
        </div>
      </div>
    </div>
  );
